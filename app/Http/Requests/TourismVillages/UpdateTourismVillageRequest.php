...
class UpdateTourismVillageRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'media.*.type' => ['required_with:media', 'string', Rule::in(['image', 'video', 'url'])],
            ...
            'worker_types.*.id' => ['nullable', 'integer'],
            'worker_types.*.type' => ['required', 'string', Rule::in(['full-time', 'part-time'])],
            'worker_types.*.amount' => ['required', 'integer', 'min:0'],
            'worker_genders' => ['nullable', 'array'],
            'worker_genders.*.id' => ['nullable', 'integer'],
            'worker_genders.*.gender' => ['required', 'string', Rule::in(['male', 'female', 'unspecified'])],
            'worker_genders.*.amount' => ['required', 'integer', 'min:0'],
            'worker_ages' => ['nullable', 'array'],
            'worker_ages.*.id' => ['nullable', 'integer'],
            'worker_ages.*.age_min' => ['nullable', 'integer', 'between:0,120'],
            'worker_ages.*.age_max' => ['nullable', 'integer', 'between:0,120', 'gte:worker_ages.*.age_min'],
            ...
            'institutionals.*.title' => ['required', 'string', 'max:150'],
            'institutionals.*.description' => ['required', 'string', 'max:255'],
            'administrator_languages' => ['nullable', 'array'],
            'administrator_languages.*.id' => ['nullable', 'integer'],
            'administrator_languages.*.language_name' => ['required', 'string', 'max:100'],
            'administrator_languages.*.proficiency_level' => ['required', 'string', Rule::in(['basic', 'intermediate', 'advanced', 'fluent'])],
            'administrator_languages.*.amount' => ['required', 'integer', 'min:0'],
            'administrator_languages.*.notes' => ['nullable', 'string'],
            'stakeholders' => ['nullable', 'array'],
            'stakeholders.*.id' => ['nullable', 'integer'],
            'stakeholders.*.name' => ['required', 'string', 'max:150'],
            'stakeholders.*.position' => ['required', 'string', 'max:150'],
        ];
    }
}
