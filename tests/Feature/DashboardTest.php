...
test('dashboard top turnovers use annual turnover data and recent assignments omit removed columns', function () {
    ...
    $this->actingAs($user)
        ...
        ->assertInertia(fn (Assert $page) => $page
            ...
            ->where('turnover_details.umkm.total', 39000000)
            ->where('turnover_details.umkm.rows.0.name', 'UMKM B')
            ->where('turnover_details.umkm.rows.0.year', 2025)
            ->where('turnover_details.umkm.rows.3.name', 'UMKM A')
            ->where('turnover_details.pariwisata.total', 44000000)
            ->where('turnover_details.pariwisata.rows.0.name', 'Wisata A')
            ->where('turnover_details.pariwisata.rows.2.name', 'Wisata C')
