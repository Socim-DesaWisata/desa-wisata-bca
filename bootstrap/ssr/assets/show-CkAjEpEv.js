...
function VillageProfileSummary({ workerTypes, stakeholders, institutionals }) {
	const totalWorkers = workerTypes.reduce((total, item) => total + item.amount, 0);
	const partTimeWorkers = workerTypes.filter((item) => item.type === "part-time").reduce((total, item) => total + item.amount, 0);
	const fullTimeWorkers = workerTypes.filter((item) => item.type === "full-time").reduce((total, item) => total + item.amount, 0);
	const summaryStats = [
		{
			value: totalWorkers,
			unit: "Orang",
...
function WorkforceSidebarSummary({ workerTypes, workerGenders, workerAges, workerEducations, administratorLanguages }) {
	const totalWorkers = workerTypes.reduce((total, item) => total + item.amount, 0);
	...
	const genderRows = [{
		...
		...gender,
		amount: workerGenders.filter((item) => item.gender === gender.value).reduce((total, item) => total + item.amount, 0)
	}));
	...
	const totalEducations = workerEducations.reduce((total, item) => total + item.amount, 0);
	...
	const educationRows = Array.from(workerEducations.reduce((groups, item) => {
		...
		groups.set(key, current);
		return groups;
	}, /* @__PURE__ */ new Map()).values()).sort((a, b) => b.amount - a.amount).map((row, index) => ({
		...row,
		color: educationColors[index % educationColors.length]
	}));
	let currentEduPercent = 0;
	const educationGradient = educationRows.length > 0 ? `conic-gradient(${educationRows.map((row, i, arr) => {
	...
	return /* @__PURE__ */ jsxs("div", {
		...
		children: [
			...
			/* @__PURE__ */ jsx(SidebarCard, {
				...
				icon: Trophy,
				children: educationRows.length ? /* @__PURE__ */ jsxs(Fragment, { children: [/* @__PURE__ */ jsxs("div", {
					className: "grid grid-cols-[112px_1fr] items-center gap-4",
					children: [/* @__PURE__ */ jsx("div", {
						className: "relative mx-auto grid size-24 place-items-center rounded-full",
...
//# sourceMappingURL=show-CkAjEpEv.js.map