                    ...
                    <div className="relative mx-auto grid min-h-[720px] max-w-[1180px] grid-cols-1 items-center gap-12 px-6 py-20 lg:grid-cols-[minmax(0,1fr)_410px]">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1">
                            <div className="grid grid-cols-2 gap-4">
                                <StatTile value="12" label="Destinasi" />
                                <StatTile value="8" label="Homestay" />
                                <StatTile value="24" label="Produk Suvenir" />
                                <StatTile
                                    value="4.8"
                                    label="Rating Pengunjung"
                                />
                            </div>
                            <div className="rounded-3xl border border-white/18 bg-white/16 p-5 text-white backdrop-blur-md">
                                <div className="flex items-center justify-between">
                                    <p className="text-sm font-black">
                                        Paket kunjungan populer
                                    </p>
                                    <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-[#0066AE]">
                                        2H1M
                                    </span>
                                </div>
                                <p className="mt-3 text-sm leading-6 font-medium text-white/82">
                                    Tur sawah, workshop suvenir, kuliner lokal,
                                    dan homestay keluarga.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="mx-auto max-w-[1180px] px-6">
                        <div className="grid gap-10 lg:grid-cols-[1fr_480px]">
                            <div>
                                <SectionHeader
                                    eyebrow="Profil desa"
                                    title="Mengenal Desa Wisata"
                                    description="Desa Wisata BCA dirancang sebagai ruang perjalanan yang rapi, ramah, dan mudah dinikmati oleh keluarga, komunitas, serta wisatawan yang mencari pengalaman lokal."
                                />
                                <p className="text-lg leading-9 font-medium text-[#475569]">
                                    {village.description ||
                                        'Desa ini menggabungkan pemandangan alam, budaya, homestay warga, kuliner, dan produk lokal dalam satu pengalaman wisata yang tertata. Setiap kunjungan mendukung ekonomi masyarakat dan menjaga keberlanjutan lingkungan.'}
                                </p>
                                <div className="mt-8 flex flex-wrap gap-3">
                                    {[
                                        ['Aman & Nyaman', ShieldCheck],
                                        ['Ramah Wisatawan', UsersRound],
                                        ['Berkelanjutan', Sparkles],
                                    ].map(([label, Icon]) => (
                                        <span
                                            key={label as string}
                                            className="inline-flex h-11 items-center gap-2 rounded-full border border-[#D7E4F1] bg-white px-5 text-sm font-black text-[#093967]"
                                        >
                            ...
                            <div className="relative overflow-hidden rounded-[32px] bg-[#093967] shadow-[0_26px_70px_rgba(9,57,103,0.18)]">
                                <img
                                    src={images.rice}
                                    alt="Profil desa wisata"
                                    className="h-[360px] w-full object-cover opacity-95"
                                />
                                <button className="absolute top-1/2 left-1/2 flex size-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-white text-[#0066AE] shadow-xl">
                                    <Play className="ml-1 size-7 fill-current" />
                                </button>
                                <div className="absolute right-5 bottom-5 left-5 rounded-2xl bg-white/94 p-5 backdrop-blur">
                                    <p className="text-sm font-black text-[#093967]">
                                        Video profil desa
                                    </p>
                                    <p className="mt-1 text-sm font-medium text-[#64748B]">
                                        Lihat gambaran perjalanan, aktivitas,
                                        dan keramahan warga desa.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="mt-10 grid gap-5 md:grid-cols-4">
                            <InfoCard
                                ...
                                title="Profil singkat desa"
                                body="Desa wisata berbasis masyarakat dengan agenda budaya dan alam."
                            />
                            <InfoCard
                                icon={<Compass className="size-6" />}
                                title="Kategori wisata"
                                body="Alam, budaya, kuliner, edukasi, dan ekonomi kreatif."
                            />
                            <InfoCard
                                icon={<Clock3 className="size-6" />}
                                title="Jam operasional"
                                body="Setiap hari, 08.00 - 20.00 WIB dengan reservasi grup."
                            />
                            <InfoCard
                                icon={<Building2 className="size-6" />}
                                title="Pengelola desa"
                                body="Dikelola bersama warga, pemandu lokal, dan pelaku UMKM."
                            />
                        </div>
                    </div>
                </section>
