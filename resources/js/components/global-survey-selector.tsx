...
export function GlobalSurveySelector() {
    ...
    useEffect(() => {
        ...
        const urlMatch = url.match(/\/survey-assignments\/([^\/?#]+)(?:\/pariwisata\/(\d+))?/);
        let currentDesaCode = urlMatch?.[1] || localStorage.getItem('selected_desa_code') || '';
        let currentPariwisataId = urlMatch?.[2] || localStorage.getItem('selected_pariwisata_id') || '';
        ...
        } else if (currentPariwisataId && currentPariwisataId !== selectedPariwisata) {
    ...
    const fetchPariwisata = (desaCode: string, preselectId?: string) => {
        ...
        fetch(`/api/dashboard/desa/${desaCode}/pariwisata`)
            ...
            .then((data: Pariwisata[]) => {
                ...
                if (preselectId && data.some(p => p.id.toString() === preselectId)) {
    ...
    const handlePariwisataChange = (id: string) => {
        ...
        if (selectedDesa) {
            router.visit(`/survey-assignments/${selectedDesa}/pariwisata/${id}`);
        }
    };

    return (
        <div className="sticky top-0 z-[999] flex w-full flex-col gap-4 border-b border-[#E5E7EB] bg-white px-4 py-3 shadow-sm sm:flex-row sm:items-center sm:justify-end sm:px-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="flex items-center gap-2">
                    <div className="relative w-48 sm:w-56">
                        <Select value={selectedDesa} onValueChange={handleDesaChange} disabled={loadingDesa}>
                            <SelectTrigger className="w-full bg-[#F8FBFE] focus:ring-1 focus:ring-[#0066AE]" aria-label="Desa Wisata">
                            ...
                            <SelectContent>
                                ...
                                {desaList.length === 0 && !loadingDesa && (
                                    <div className="px-2 py-2 text-sm text-gray-500">Tidak ada data</div>
                                )}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    <div className="relative w-48 sm:w-56">
                        ...
                        >
                            <SelectTrigger className="w-full bg-[#F8FBFE] focus:ring-1 focus:ring-[#0066AE]" aria-label="Pariwisata">
                                <div className="flex items-center gap-2 overflow-hidden">
                            ...
                            <SelectContent>
                                {pariwisataList.map(pariwisata => (
                                    <SelectItem key={pariwisata.id} value={pariwisata.id.toString()}>
                                        {pariwisata.name}
                                    </SelectItem>
                                ))}
                                {pariwisataList.length === 0 && !loadingPariwisata && selectedDesa && (
                                    <div className="px-2 py-2 text-sm text-gray-500">Tidak ada pariwisata</div>
                                )}
                            </SelectContent>
                        </Select>
                    </div>
